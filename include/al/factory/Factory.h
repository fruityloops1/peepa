...
namespace al {
    ...
    class Factory {
      public:
        inline Factory(const char* factory_name) : mName(factory_name), mEntries(nullptr), mEntryAmount(0) {}
        template <int N>
        inline Factory(const char* factory_name, al::NameToCreator<T> (&entries)[N]) : mName(factory_name) {
            initFactory(entries);
        }
        template <int N>
