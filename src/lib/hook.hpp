...
#define _HOOK_IMPL_NAME(name) APPEND(name, _IMPL_SUFFIX)
#define _HOOK_NAME(name) APPEND(name, _HOOK_SUFFIX)

#define _MAKE_HOOK_IMPL(ret, name, args, body)                                                                         \
    ret _HOOK_NAME(name) args { body }

#define MAKE_HOOK(ret, name, args, body) static _MAKE_HOOK_IMPL(ret, name, args, body)
...
#define INJECT_HOOK(offset, name) exl::hook::HookFunc(offset, _HOOK_NAME(name));
#define INJECT_HOOK_T(offset, name) _HOOK_IMPL_NAME(name) = exl::hook::HookFunc(offset, _HOOK_NAME(name), true);

#define _HOOK_CLASS_VALUE_SUFFIX ClassValue
#define _HOOK_CLASS_SUFFIX Class

#define _MAKE_CLASS_VALUE_NAME(name) APPEND(name, _HOOK_CLASS_VALUE_SUFFIX)

#define _MAKE_CLASS_NAME(name) APPEND(name, _HOOK_CLASS_SUFFIX)
...
#define _HOOK_CLASS_HOOK_NAME(name) _MAKE_CLASS_NAME(name)::_HOOK_NAME(name)

#define _HOOK_CLASS_FUNC(name, funcname) _MAKE_CLASS_VALUE_NAME(name)::funcname

#define _HOOK_CLASS_T_NAME(name) _MAKE_CLASS_NAME(name)::_HOOK_IMPL_NAME(name)
...
#define INJECT_CLASS_HOOK_T(funcname, name)                                                                            \
    _HOOK_CLASS_T_NAME(name) =                                                                                         \
        exl::util::Hook::HookFunc(&_HOOK_CLASS_FUNC(name, funcname), &_HOOK_CLASS_HOOK_NAME(name), true);

#define CPP_IMPL(...) (this->*impl)(__VA_ARGS__)
...
namespace exl::hook {
    ...
    inline uintptr_t GetSelfStart() { return util::GetSelfModuleInfo().m_Total.m_Start; }
    inline uintptr_t GetTargetOffset(uintptr_t offset) { return util::GetMainModuleInfo().m_Total.m_Start + offset; }
    inline uintptr_t GetTargetStart() { return GetTargetOffset(0); }

    inline void Initialize() { util::Hook::Initialize(); }
