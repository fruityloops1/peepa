...
private:
    inline bool DoneNothing() const { return m_Start == m_Current; }
...
protected:
    inline ptrdiff_t RelativeAddressFromBase(uintptr_t address) const { return address - m_Current; }
    inline ptrdiff_t RelativeAddressFromPointer(void* ptr) const { return AddrFromRoPointer(ptr) - m_Current; }

    uintptr_t m_Start;
    uintptr_t m_Current;

public:
    inline StreamPatcher(uintptr_t start)
        : m_Start(start)
        , m_Current(start)
    {
    }
    ...
    inline void Seek(uintptr_t address) { SeekRel(RelativeAddressFromBase(address)); }
    /* Absolute address. */
    inline void Seek(void* ptr) { SeekRel(RelativeAddressFromPointer(ptr)); }

    inline ~StreamPatcher() { Flush(); }
};
} // namespace exl::patch